...
small_DEQ_nitrate_table<-small_DEQ_nitrate%>%filter(NITRATE_PLUS_NITRITE_mg_L_N>=0)%>%mutate(Observation=1)%>%
...
small_DEQ_discharge_table<-small_DEQ_discharge%>%filter(DISCHARGE_GPM>=0)%>%mutate(Observation=1)%>%group_by(DEQ_NUMBER)%>%summarise('Median Discharge (GPM)'=median(DISCHARGE_GPM), 'Mean Discharge (GPM)'= mean(DISCHARGE_GPM), 'Max Discharge (GPM)'= max(DISCHARGE_GPM), 'Min Discharge (GPM)'=min(DISCHARGE_GPM), Observation= sum(Observation))
...
small_picture<- full_join(small_DEQ_discharge_table, small_DEQ_nitrate_table, c("DEQ_NUMBER"="DEQ_WELL_NUMBER"))%>%filter(`Median Discharge (GPM)`<= 1100,`Median Discharge (GPM)`>= 90, `Median Nitrate (mg/L)`>= 3.00)

####### THERE ARE 8 SPRINGS THAT MEET BOTH CRITERIA #######

### Finding out the info to create the bargraph 

small_DEQ_nitrate[small_DEQ_nitrate == -9999]<- NA

VA_nitrate_summary<-refined_DEQ_WQ%>%mutate(Observation= 1)%>%group_by(DEQ_WELL_NUMBER)%>%summarise('Median Nitrate (mg/L)'=median(NITRATE_PLUS_NITRITE_mg_L_N, na.rm = TRUE), 'Mean Nitrate (mg/L)'= mean(NITRATE_PLUS_NITRITE_mg_L_N, na.rm = TRUE), 'Max Nitrate (mg/L)'= max(NITRATE_PLUS_NITRITE_mg_L_N, na.rm = TRUE), 'Min nitrate (mg/L)'=min(NITRATE_PLUS_NITRITE_mg_L_N, na.rm = TRUE), Observation= sum(Observation))


refined_DEQ_discharge[refined_DEQ_discharge == -9999]<- NA
